        ...
        <div id="filtros" class="flex flex-wrap gap-2 border-b border-graphite/10 pb-6">
            @foreach ($categorias as $slug => $info)
                <button data-filter="{{ $slug }}"
                        class="filtro-btn px-4 py-2 rounded-md text-sm font-medium transition
            ...
            @foreach ($categorias as $slug => $info)
                @for ($i = 1; $i <= 4; $i++)
                    ...
                    <div class="galeria-card rounded-lg overflow-hidden bg-white shadow-sm border border-graphite/5 transition hover:shadow-md hover:-translate-y-0.5 {{ $slug === $primeiraCategoria ? '' : 'hidden' }}"
                         data-category="{{ $slug }}">
                        @if ($existe)
                            <div class="aspect-video bg-cover bg-center" style="background-image: url('{{ asset($caminhoRelativo) }}');"></div>
                        @else
                            <div class="aspect-video bg-graphite-light/10 animate-pulse flex items-center justify-center text-xs text-graphite/40">
                                Em breve
                            </div>
                        @endif
                        <div class="p-4">
